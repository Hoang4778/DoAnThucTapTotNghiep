...
const quotationTypes = document.querySelectorAll("input[name='quotation-type']")
const selectSetting = document.querySelector(".select-setting")
const hasGlobalDiscount = document.querySelector(".checkbox-has-global-discount")
const globalDiscountAmount = document.querySelector(".input-global-discount")
const hasGlobalSurcharge = document.querySelector(".checkbox-has-global-surcharge")
...
const hasPageBreakBeforeTable = document.querySelector(".checkbox-page-break-before-quotation-table")
const displayCustomerDetails = document.querySelector(".checkbox-display-customer-details")
const displayQuantityAndPrice = document.querySelector(".checkbox-display-quantity-and-unit-price")
const hasPageBreakAfterTable = document.querySelector(".checkbox-page-break-after-quotation-table")
const showQuotationStartDate = document.querySelector(".checkbox-quotation-start-date")
const showQuotationExpiryDate = document.querySelector(".checkbox-quotation-expiration-date")
const signatureBoxes = document.querySelectorAll("input[name='signature-box']")
const quotationFooterPositions = document.querySelectorAll("input[name='quotation-footer-position']")
const selectLetterTopTemplate = document.querySelector(".select-letter-top-template")
        ...
        fetch(
            ...
            .then((result) => {
                if (result.isOkay) {
                    ...
                    if (branchSetting.globalDiscountAmount != 0) {
                    ...
                    if (branchSetting.globalSurchargeAmount != 0) {
...
btnSaveSetting.onclick = (e) => {
    ...
    const settingForm = document.querySelector(".settings-form")
    const selectBranch = document.querySelector(".select-branch");
    const quotationType = document.querySelector("input[name='quotation-type']:checked")
    const defaultTaxRate = document.querySelector(".input-default-tax-rate")
    const selectSetting = document.querySelector(".select-setting")
    const hasGlobalDiscount = document.querySelector(".checkbox-has-global-discount")
    const globalDiscountAmount = document.querySelector(".input-global-discount")
    ...
    const showQuotationStartDate = document.querySelector(".checkbox-quotation-start-date")
    const showQuotationExpiryDate = document.querySelector(".checkbox-quotation-expiration-date")
    const signatureBox = document.querySelector("input[name='signature-box']:checked")
    const quotationFooterPosition = document.querySelector("input[name='quotation-footer-position']:checked")
    const selectLetterTopTemplate = document.querySelector(".select-letter-top-template")
    ...
    if (isNaN(parseFloat(defaultTaxRate.value))) {
        ...
        return
    } else if (parseFloat(defaultTaxRate.value) > 100 || parseFloat(defaultTaxRate.value) < 0) {
        loadingPart.classList.remove("active");
        showPopup("The default tax rate is not from 0% to 100%. Please put a suitable number.")
        return
    }
    ...
    if (hasGlobalDiscount.checked) {
        if (isNaN(parseFloat(globalDiscountAmount.value))) {
            ...
            return
        } else if (parseFloat(globalDiscountAmount.value) > 100 || parseFloat(globalDiscountAmount.value) < 0) {
            loadingPart.classList.remove("active");
            showPopup("The global discount is not from 0% to 100%. Please put a suitable number.")
            return
        }
    }
    ...
    if (hasGlobalSurcharge.checked) {
        if (isNaN(parseFloat(globalSurchargeAmount.value))) {
            ...
            return
        } else if (parseFloat(globalSurchargeAmount.value) > 100 || parseFloat(globalSurchargeAmount.value) < 0) {
            loadingPart.classList.remove("active");
            showPopup("The global surcharge is not from 0% to 100%. Please put a suitable number.")
            return
        }
    }

    const setting = {
        id: settingForm.dataset.settingId != undefined ? parseInt(settingForm.dataset.settingId) : null,
        branchCode: selectBranch.value,
        quotationTypeId: quotationType != null ? parseInt(quotationType.value) : null,
        defaultTaxRate: parseFloat(parseFloat(defaultTaxRate.value).toFixed(2)),
        headerFooterSettingCode: selectSetting.value != "" ? selectSetting.value : null,
        hasGlobalDiscount: hasGlobalDiscount.checked,
        globalDiscountAmount: hasGlobalDiscount.checked ? parseFloat(globalDiscountAmount.value) : null,
        hasGlobalSurcharge: hasGlobalSurcharge.checked,
        globalSurchargeAmount: hasGlobalSurcharge.checked ? parseFloat(globalSurchargeAmount.value) : null,
        ...
        customerAcceptanceSignatureBoxId: signatureBox != null ? parseInt(signatureBox.value) : null,
        quotationFooterPositionId: quotationFooterPosition != null ? parseInt(quotationFooterPosition.value) : null,
        letterTopTemplateId: selectLetterTopTemplate.value != "" ? parseInt(selectLetterTopTemplate.value) : null
    }
